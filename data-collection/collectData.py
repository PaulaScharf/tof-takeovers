...
# Um den Code auszuführen wird der VL53L8CX-Sensor und eine Webcam benötigt


# Aufbau der seriellen Schnittstelle, ggf. anpassen
...
with open('data-collection/recording.csv', 'w', newline='') as file:
...
def get_sensor_data(timestamp):
    ...
    try:
        ...
        if data:
            global letzteMessung
            aktuelleZeit = round(time.time() * 1000)
            print(round(1000/(aktuelleZeit-letzteMessung)))
            ...
            if len(distances) == 64:
                ...
                if len(matritzen) == 20:
                    ...
                    with open('data-collection/recording.csv', 'a', newline='') as file:
                        writer = csv.writer(file)
                        writer.writerow(row)
...
# ggfs. Zahl anpassen, wenn Webcam genutzt wird
